...
/**
 * Высота потолка: 2..4 метра. Нижняя граница — жилой минимум, верхняя
 * отсекает опечатку «27» вместо «2,7». Число новосёл берёт из плана
 * квартиры, а не меряет.
 */
export const CeilingMeters = z
  .number()
  .gte(2)
  .lte(4)
  .refine((value) => Number(value.toFixed(2)) === value, 'Не больше двух знаков после запятой')
...
/**
 * Глубина ниши: >0, ≤2 метра. Единственная глубина, которую спрашивают —
 * она ограничивает шкаф физически. Глубину кухни и тумбы в ванной задаёт
 * мастер, и вопрос о ней дал бы шум вместо данных (решение 18.09).
 */
export const NicheDepthMeters = z
  .number()
  .gt(0)
  .lte(2)
  .refine((value) => Number(value.toFixed(2)) === value, 'Не больше двух знаков после запятой')

...
/**
 * Карточка в том виде, в каком она уходит наружу. Контакт вырезан схемой,
 * а не разметкой: `getMasterCard` и `listMasters` открыты без токена (§5),
 * и поле, спрятанное только на экране, всё равно лежит в теле ответа —
 * его видно в консоли браузера. Телефон мастерской — ПДн (§2).
 *
 * `messengers` уходит вместе с ним: без номера «отвечаем в WhatsApp»
 * не значит ничего, кроме лишней строки о человеке.
 */
export const PublicMasterCard = MasterCard.omit({ contactPhone: true, messengers: true })

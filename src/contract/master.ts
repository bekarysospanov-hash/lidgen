...
export const MasterCard = z.object({
  ...
  /**
   * Два факта, которые заказчик ищет первым делом (решение PM 21.09,
   * по разбору карточек Kaspi, OLX и Яндекс.Услуг — там оба вынесены
   * в описание каждым вторым объявлением).
   *
   * `worksByContract` — договор. Человек отдаёт предоплату за предмет,
   * которого ещё нет, незнакомой мастерской; договор — первое, чем этот
   * страх снимается.
   *
   * `ownProduction` — свой цех, без посредников. Отвечает на второй
   * страх: «мой заказ перепродадут неизвестно кому, и спрашивать будет
   * не с кого».
   *
   * Булевы, а не текст: оба проверяются одним вопросом при заведении
   * карточки (A2) и сравниваются между мастерскими. Рассрочки здесь нет
   * намеренно — она уже живёт услугой `installments` с условием,
   * и второй источник правды о ней завести нельзя.
   */
  worksByContract: z.boolean().default(false),
  ownProduction: z.boolean().default(false),
  /**
 ...
 * текст, направления, услуги, отличия, снимки работ, логотип, гарантия, срок,
 * часы и контакт.
 *

...
export default function Masters() {
  ...
  /**
   * Отбор по виду работ идёт по снимкам, а не по строке «Делает»: снимок
   * с видом — структурное поле карточки, а направления мастерская пишет
   * своими словами, и «кухни под ключ» с «Кухни» не совпадут никогда.
   */
  const shown = view.masters.filter(
    (master) =>
      (city === null || master.city.code === city) &&
      (kind === null || master.card.photos.some((photo) => photo.kind === kind)),
  )

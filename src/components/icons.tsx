...
/**
 * Расчёт по заявке — лист с ценой (21.09). Предметный контур, а не абстракция:
 * человек видел такой лист у мебельщика на замере. Подпись рядом стоит
 * в любом случае — иконка без подписи в системе запрещена.
 */
...
/**
 * Безопасная покупка — щит с монетой внутри. От гарантии (щит с галочкой)
 * отличается содержимым: там «сделаем как обещали», здесь «деньги под
 * присмотром».
 */
export function SafeDealIcon() {
  return (
    <Glyph>
      <path d="M12 3l7 2.6v5.6c0 4-2.9 7.4-7 8.8-4.1-1.4-7-4.8-7-8.8V5.6z" />

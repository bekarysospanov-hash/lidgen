...
async function ответить(
  ...
  // Чипсы нажимаются по видимой подписи: сам checkbox sr-only, и Playwright
  // его не нажмёт — та же причина, что у строк выбора в форме заказчицы.
...
test('заявка доходит от формы до вилки на странице заказчицы @shots', async ({ page }) => {
  ...
  await expect(page.getByText('Что входит в цену?')).toBeVisible()
  ...
  await ответить(
    ...
    '1250000',
    '30',
    ['Корпуса и полки', 'Дверцы', 'Столешница', 'Выдвижные ящики', 'Доставка'],
